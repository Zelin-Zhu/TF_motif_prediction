...
y_0_250=np.zeros((len(seqs_0_250),5))
y_0_250[:,0]=1
y_250_500=np.zeros((len(seqs_250_500),5))
y_250_500[:,1]=1
y_500_750=np.zeros((len(seqs_500_750),5))
y_500_750[:,2]=1
y_750_1000=np.zeros((len(seqs_750_1000),5))
y_750_1000[:,3]=1
y_background=np.zeros((len(seqs_background),5))
y_background[:,4]=1



x=np.append(seqs_0_250,seqs_250_500,axis=0)
x=np.append(x,seqs_500_750,axis=0)
x=np.append(x,seqs_750_1000,axis=0)
x=np.append(x,seqs_background[0:2000],axis=0)

y=np.append(y_0_250,y_250_500,axis=0)
y=np.append(y,y_500_750,axis=0)
y=np.append(y,y_750_1000,axis=0)
y=np.append(y,y_background[0:2000],axis=0)
...
model.save('D:/workspace of spyder/毕业设计/my project data/model_file/multi_classification_DNd41.h5') 
# 加载模型，同时加载了模型的结构、权重等信息
loaded_model = tf.keras.models.load_model('D:/workspace of spyder/毕业设计/my project data/model_file/multi_classification_DNd41.h5')
